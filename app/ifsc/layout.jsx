...
export default function IFSCForm() {
  ...
  useEffect(() => {
    ...
    if (bank && !stateVal) {
      ...
      console.log("bank", banko);

      router.push(`/ifsc/${banko}`);
      return;
    }

    // Bank, State => e.g. "/SBIN/andhra-pradesh"
    if (bank && stateVal && !cityVal) {
      router.push(`/ifsc/${bank}/${toSlug(stateVal)}`);
      return;
    }

    // Bank, State, City => e.g. "/SBIN/andhra-pradesh/vijayawada"
    if (bank && stateVal && cityVal && !branchVal) {
      router.push(`/ifsc/${bank}/${toSlug(stateVal)}/${toSlug(cityVal)}`);
      return;
    }

    // Bank, State, City, Branch => e.g. "/SBIN/andhra-pradesh/vijayawada/m-g-road"
    if (bank && stateVal && cityVal && branchVal) {
      router.push(`/ifsc/${bank}/${toSlug(stateVal)}/${toSlug(cityVal)}/${toSlug(branchVal)}`);
    }
